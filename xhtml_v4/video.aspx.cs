...
namespace xhtml
{
	public class video : XCatalogBrowsing
	{
			...
			if(_idContent > 0)
			{
				try 
				{
					...
					if (op.OperatorName!= null && op.OperatorName == "MOVISTAR")
					{
						...
						downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_VIDEO_CLIP"], null, WrapperType.DescriptorWrapper);
				...
				{
					WapTools.SendMail("video", Request.UserAgent, caught.ToString(), Request.ServerVariables);				
					Log.LogError(String.Format("Site emocion : Unexpected exception in emocion\\xhtml\\video.aspx - UA : {0} - QueryString : {1}", Request.UserAgent, Request.ServerVariables["QUERY_STRING"]), caught);
