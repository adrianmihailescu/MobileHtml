...
namespace xhtml
{
	public class promo : XCatalogBrowsing
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(_idContent > 0)
			{
				try 
				{
					...
					if (op.OperatorName!= null && op.OperatorName == "MOVISTAR")
					{
						...
						if (_idContentSet == 5790 || _idContentSet == 5648 || _idContentSet == 5793 || _idContentSet == 5770 || _idContentSet == 5781 || _idContentSet == 5783 || _idContentSet == 5784 || _idContentSet == 2090)
							downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_IMG_COLOR_PROMO"], null, WrapperType.DescriptorWrapper | WrapperType.ForwardLockWrapper);
						else
							downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_IMG_COLOR_PROMO"], null, WrapperType.DescriptorWrapper);
				...
				{
					WapTools.SendMail("promo", Request.UserAgent, caught.ToString(), Request.ServerVariables);				
					Log.LogError(String.Format("Site emocion : Unexpected exception in emocion\\xhtml\\promo.aspx - UA : {0} - QueryString : {1}", Request.UserAgent, Request.ServerVariables["QUERY_STRING"]), caught);
