...
namespace xhtml
{
	public class videobranded : XCatalogBrowsing
	{
			...
			if(_idContent > 0)
			{
				try 
				{
					...
					if (op.OperatorName!= null && op.OperatorName == "MOVISTAR")
					{
						...
						downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_VIDEO_BRANDED"], null, WrapperType.DescriptorWrapper);
						...
						row = new XhtmlTableRow();
						XhtmlTools.AddTextTableRow("", row, "", "Puede durar hasta un minuto", Color.Empty, Color.Empty, 1, HorizontalAlign.Center, VerticalAlign.Middle, false, FontUnit.XXSmall);
						tbContent.Rows.Add(row);				
				...
				{
					WapTools.SendMail("videobranded", Request.UserAgent, caught.ToString(), Request.ServerVariables);
					Log.LogError(String.Format("Site emocion : Unexpected exception in emocion\\xhtml\\videobranded.aspx - UA : {0} - QueryString : {1}", Request.UserAgent, Request.ServerVariables["QUERY_STRING"]), caught);
