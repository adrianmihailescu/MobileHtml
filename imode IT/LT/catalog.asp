...
Function Display()
	...
	If l_strApp <> "" Then
		...
		l_strUrl = "catalog.asp?cg=" & g_strContentGroup & "&ct=" & g_strContentType & "&c=" & g_lngIDContentSet & "&cgd=" & g_strContentGroupDisplay & "&v=" & g_bytViewImg
		...
		For l_lngIdx = g_lngItemPerPage * (g_lngNumPage - 1) + 1 To g_lngItemPerPage * g_lngNumPage
			...
			'Response.Write(g_strContentGroupDisplay & g_objDico(g_strContentGroupDisplay & "_COLOR_DARK"))
				l_strHTML = l_strHTML & _
						"<tr><td><font color='" & g_objDico(g_strContentGroupDisplay & "_COLOR_DARK") & "'><li>&nbsp;<a href='catalog.asp?cg=" & l_arrContent(cst_fldCompositeContentGroup) & "&ct=" & GetDefaultType(l_arrContent(cst_fldCompositeContentGroup)) & "&c=" & l_arrContent(cst_fldIDComposite) & "&cgd=" & g_strContentGroupDisplay & "&b=" & Server.URLEncode(l_strUrl & "&n=" & g_lngNumPage) & "'>" & Server.HTMLEncode(l_arrContent(cst_fldName)) & "</a></font></td></tr>" & vbCrLf
			Else			
				l_strMimeType = "GIF"	
...
<div align="center">
	<img src="../images/lt<% = Common_GetMobileWidth() %>.gif" alt="Looney Tunes"><br></div>
	<table width="100%" cellspacing="0" bgcolor="<% = g_objDico(g_strContentGroupDisplay & "_COLOR_DARK") %>">
		<tr>
			<td align="center">
				<font color="<% = g_objDico(g_strContentGroupDisplay & "_COLOR_LIGHT") %>"><% = g_objDico(g_strContentGroupDisplay & "_EMOJI") & " " & g_objDico(g_strContentGroupDisplay) %>: <% = Server.HTMLEncode(g_strContentSetName) %></font>
			</td>
		</tr>
	</table>
	<table width="100%" border="0" cellspacing="0" cellpadding="1">
		...
		<tr>
			...
			<% If g_bytViewImg = 1 Then %> 
				<a href="catalog.asp?c=<% = g_lngIDContentSet %>&cg=<% = g_strContentGroupDisplay %>&ct=<% = g_strContentType %>&n=<% = g_lngNumPageOtherDisplay %>&v=0">Visualizza per lista</a> 
			<% Else %> 
				<a href="catalog.asp?c=<% = g_lngIDContentSet %>&v=1&cg=<% = g_strContentGroupDisplay %>&ct=<% = g_strContentType %>&n=<% = g_lngNumPageOtherDisplay %>">Visualizza le anteprime</a> 
			<% End If %>
			</td>
		</tr>
		<% End If %>
