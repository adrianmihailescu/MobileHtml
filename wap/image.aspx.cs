...
namespace wap
{
	...
	/// </summary>
	public class image : CatalogBrowsing
	{
			...
			if(_idContent > 0)
			{
				try 
				{
					...
					if (op.OperatorName!= null && op.OperatorName == "MOVISTAR")
					{
						...
						if (_idContentSet == 5790 || _idContentSet == 5648 || _idContentSet == 5793 || _idContentSet == 5770 || _idContentSet == 5781 || _idContentSet == 5783 || _idContentSet == 5784 || _idContentSet == 2090 || _idContentSet == 6162)
							downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_IMG_COLOR"], null, WrapperType.DescriptorWrapper | WrapperType.ForwardLockWrapper);
						else
							downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_IMG_COLOR"], null, WrapperType.DescriptorWrapper);
						...
						/*		if (contentSet.Name.ToUpper().IndexOf("MÁS ") < 0)
									WapTools.AddLink(pnlContent, "Más " + contentSet.Name, String.Format("http://emocion.kiwee.com/wap/catalog.aspx?cs={0}&cg={1}", _idContentSet.ToString(), _contentGroup), "", _mobile);
								else
									WapTools.AddLink(pnlContent, contentSet.Name, String.Format("http://emocion.kiwee.com/wap/catalog.aspx?cs={0}&cg={1}", _idContentSet.ToString(), _contentGroup), "", _mobile);
						*/	
							}
							catch{}
						}						
				...
				{
					WapTools.SendMail("image", Request.UserAgent, caught.ToString(), Request.ServerVariables);
					Log.LogError(String.Format(" Unexpected exception in EMOCION --> emocion.kiwee.com\\wap\\image.aspx - UA : {0} - QueryString : {1}", Request.UserAgent, Request.ServerVariables["QUERY_STRING"]), caught);
