...
namespace wap
{
	...
	/// </summary>
	public class animation : CatalogBrowsing
	{
			...
			if(_idContent > 0)
			{
				try 
				{
					...
					if (op.OperatorName!= null && op.OperatorName == "MOVISTAR")
					{
						...
						downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_ANIM_COLOR"], null, WrapperType.DescriptorWrapper);
				...
				{
					WapTools.SendMail("animation", Request.UserAgent, caught.ToString(), Request.ServerVariables);
					Log.LogError(String.Format(" Unexpected exception in EMOCION --> emocion.kiwee.com\\wap\\animation.aspx - UA : {0} - QueryString : {1}", Request.UserAgent, Request.ServerVariables["QUERY_STRING"]), caught);
