...
namespace wap
{
	...
	/// </summary>
	public class animationbranded : CatalogBrowsing
	{
			...
			if(_idContent > 0)
			{
				try 
				{
					...
					if (op.OperatorName!= null && op.OperatorName == "MOVISTAR")
					{
						...
						downloadInfo = billingManager.DeliverCommand(Request, billingRequest.GUIDCommand, ConfigurationSettings.AppSettings["UrlContentWap_ANIM_BRANDED"], null, WrapperType.DescriptorWrapper);
				...
				{
					WapTools.SendMail("animationbranded", Request.UserAgent, caught.ToString(), Request.ServerVariables);
					Log.LogError(String.Format(" Unexpected exception in EMOCION --> emocion.kiwee.com\\wap\\animationbranded.aspx - UA : {0} - QueryString : {1}", Request.UserAgent, Request.ServerVariables["QUERY_STRING"]), caught);
